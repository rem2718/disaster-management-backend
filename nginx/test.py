...
def stream(name, width, height):
    rtmp_url = f'rtmp://127.0.0.1/live/re-{name}'
    ffmpeg_command = [
        'ffmpeg',
        '-y', 
        '-f', 'rawvideo',  
...
def test(name):
    url = f"rtmp://127.0.0.1/live/{name}"
    ...
    try:
        process = stream(name, width, height)
        while True:
            ret, frame = cap.read()
            if not ret:
...
parser = argparse.ArgumentParser(description="Stream video to an RTMP server using OpenCV and FFmpeg.")
